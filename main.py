    ...
    if multiLandMarks: # если руки найдены
        ...
        for idx, handLms in enumerate(multiLandMarks): # перебираем найденные руки
            ...
            if fingersList[5][0] > fingersList[17][0]:
                side = 'right'
            if side == 'left':
                if fingersList[thumb_coord[0]][0] < fingersList[thumb_coord[1]][1]:
                    upCount += 1
            else:
                if fingersList[thumb_coord[0]][0] < fingersList[thumb_coord[1]][1]:
                    upCount += 1
        cv2.putText(image, str(upCount), (50, 100), cv2.FONT_ITALIC, 5, (50,200,255), 5)
        print(upCount)

    currentTime=time.time()
    fps = 1 / (currentTime - prevTime)
    cv2.putText(image, f"FPS: {fps}", (150,100),cv2.FONT_ITALIC, 3, (200,200,200), 3)
